(function () {
  ...
  angular.module('ew').config(function ($stateProvider, $urlRouterProvider, $locationProvider, USER_ROLES) {
    ...
    $stateProvider
      ...
      .state('cache', new State(USER_ROLES, 'cache', 'component/cache/cache.html').roles([USER_ROLES.admin]).controller('CacheCtrl').build())
      ...
      .state('productioncat', new State(USER_ROLES, 'productioncat/:code', 'component/production-category/production-category.html').roles([USER_ROLES.admin]).controller('ProductionCategoryCtrl').build())
